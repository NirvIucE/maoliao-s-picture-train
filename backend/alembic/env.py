...
Date: 2026-08-20 20:14:44
LastEditors: NirvIucE 1750682685@qq.com
LastEditTime: 2026-08-20 20:20:49
FilePath: \new-picture-train\backend\alembic\env.py
...
# 让 alembic 能找到 src 包（把 backend 目录加入 sys.path）
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from src.config import DATABASE_URL
from src.database import Base
# 导入所有模型，确保它们注册到 Base.metadata（autogenerate 依赖）
...
# 从项目 config.py 读取数据库 URL，替代 alembic.ini 里的占位符
